...
public class StockChecker {
	...
	/**
	 * This method does not use the conventional contains method of Java and it
	 * first sorts the elements in ascending order and checks in case the element is
	 * less than the first element or greater than the last element i.e. the element
	 * is not present in list. Then it checks the difference of the entered element
	 * with first element and last element . If the first difference is smaller than
	 * the second difference, then it starts liner search from the sorted list in
	 * ascending order. But in case the difference is found to be larger, then it
	 * sorts the list again in descending order and performs the linear search
	 * 
	 * @param myObj
